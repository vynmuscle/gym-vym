...
import { authenticateRequest } from './_aiAuth.js';
import { MODEL_NAME } from './_aiConfig.js';
...
export default async function handler(req, res) {
  ...
  const allowed = await checkRateLimit(userId, 'ai-workout', 20);
  if (!allowed) return res.status(429).json({ error: 'Limite diário de gerações por IA atingido. Tente novamente amanhã.' });
